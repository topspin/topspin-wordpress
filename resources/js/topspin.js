var topspin = {
	...
	registerEvents : function() {
		// Bind lightbox
		jQuery('.topspin-viewbox').live('click', topspin.onClickViewBox);
		
		// Bind Gallery Pager
		jQuery('.topspin-gallery-pager-image').live('click', topspin.onClickGalleryPagerImage);
	},
	...
	viewMore : function(offer_id) {
		...
		jQuery.getJSON(topspin.opts.ajaxurl, data, topspin.onViewMoreJSONCallback);
	},
	...
	onViewMoreJSONCallback : function(ret) {
		if(ret.status=='success') {
			jQuery.fancybox.open({
				...
				afterShow : topspin.onViewAfterShow
			});
		}
	},
